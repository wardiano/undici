...
class Parser extends HTTPParser {
  ...
  [NodeHTTPParser.kOnHeadersComplete] (rawHeaders, statusCode, upgrade, shouldKeepAlive) {
    /* istanbul ignore next: difficult to make a test case for */
    if (this.paused) {
      this.queue.push([this[NodeHTTPParser.kOnHeadersComplete], rawHeaders, statusCode, upgrade, shouldKeepAlive])
      return
    }
    ...
    if (request.upgrade !== true && Boolean(upgrade) !== Boolean(request.upgrade)) {
  ...
  [NodeHTTPParser.kOnBody] (chunk) {
    if (this.paused) {
      this.queue.push([this[NodeHTTPParser.kOnBody], chunk])
      return
    }
